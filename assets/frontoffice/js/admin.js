...
(function () {
  ...
  var DAYS = ["01.03", "06.03", "11.03", "16.03", "21.03", "26.03", "31.03"];
  ...
  var MONTHS_HALF = ["Jan", "Feb", "Mar", "Apr", "May", "Jun"];

  //New and Returned visitors
  var $canvasesMonthlyVisitors = jQuery(
    ".canvas-chart-line-new-returned-visitors"
  );
          ...
          datasets: [
            {
              ...
              tension: "0",
              //visitors per month
              data: [446, 243, 544, 645, 443, 437, 750],
              fill: true,
            },
            //put new dataset here if needed to show multiple datasets on one graph
          ],
          ...
          datasets: [
            {
              ...
              data: [1.2, 1.5, 1.3, 1.9, 2, 2.6, 2.4],
              fill: true,
            },
            //put new dataset here if needed to show multiple datasets on one graph
          ],
    ...
    $canvasesMonthlyPieVisitors.each(function (i) {
      var config = {
        ...
        data: {
          ...
          labels: ["New Visitors", "Returned Visitors"],
        },
      };
  ...
  fetch("../../controller/calendar.php")
    .then((response) => response.json())
    .then((data) => {
      var currentDate = new Date(); // Get the current date

      var appo = data.map((event) => ({
        title: event.name,
        start: event.date.substring(0, 10),
        type: event.type, // Assuming the fetched data has a 'type' property
      }));

      // Configure FullCalendar after fetching and formatting the data
      jQuery(".events_calendar").fullCalendar({
        ...
        eventRender: function (event, element) {
          ...
          // Check if event's date hasn't passed
          var eventDate = new Date(event.start);
          if (eventDate >= currentDate) {
            // Change background color based on event type
            if (event.type === "online") {
              element.css("background-color", "#4DB19Eff");
            } else if (event.type === "offline") {
              element.css("background-color", "#EBB26Aff");
            }
            // Add more conditions as needed for other types
          } else {
            // Set a different color for past events
            element.css("background-color", "#787878"); // For example, grey for past events
          }
        },
      });
    })
  ...
  (function () {
    ...
    function cb(start, end) {
      jQuery(".dashboard-daterangepicker span").html(
