...
$event = new event();
$idevent = isset($_GET["id"]) ? $_GET["id"] : null;
$nomevent = $descriptionevent = $nbevent = $image = ''; // Initialisation de la variable $image

if (!empty($idevent)) {
    $tab = $event->afficherevent($idevent);

        $nomevent = $tab["nomevent"];
        $descriptionevent = $tab["descriptionevent"];
        $nbevent = $tab["nbevent"];
        $image = $tab["image"];
		$date = date($tab["dateevent"]);
		$loc = $tab["location"];
		$prix = $tab["prix"];
...
if (isset($_POST["update"])) {
    $new_nomevent = $_POST["nom"];
    $new_descriptionevent = $_POST["desc"];
    $new_nbevent = $_POST["nb"];
	$date = $_POST["date"];
	$prix = $_POST["prix"];
    $event->updateevent($idevent, $new_descriptionevent, $new_nbevent, $new_nomevent,$_POST["location"],$date,$prix);
    //header('Location: backview.php');
}
?>
<!DOCTYPE html>
<html lang="fr">
...
<style>
    ...
    <script>
        function validateForm() {
            ...
            // Name validation
            var nom = document.forms["formUpdate"]["nom"].value;
            ...
            // Nombre validation
            var nb = document.forms["formUpdate"]["nb"].value;
            if (!/^\d+$/.test(nb)) {
            ...
            // Location validation
            var location = document.forms["formUpdate"]["location"].value;
            ...
            var selectedDate = new Date(document.forms["formUpdate"]["date"].value);
            if (selectedDate < currentDate || document.forms["formUpdate"]["date"].value == "") {
            ...
            if (prix == "") {
                document.getElementById("prixError").innerText = "price is required.";
                isValid = false;
            } else {
                document.getElementById("prixError").innerText = "";
            }

            return isValid;
        }
    </script>
    <form action="" method="post" id="formUpdate" onsubmit="return validateForm()">
        <label for="">Nomevent :</label>
        <input type="text" name="nom" value="<?php echo $nomevent; ?>"><br>
		<span id="nomError" class="error"></span><br>
        <label for="">Description :</label>
        <input type="text" name="desc" value="<?php echo $descriptionevent; ?>"><br>
		<span id="descError" class="error"></span><br>
        <label for="">Nombre de places :</label>
        <input type="text" name="nb" value="<?php echo $nbevent; ?>"><br>
		<span id="nbError" class="error"></span><br>
		...
		<br>
		<span id="prixError" class="error"></span><br>
        <!-- Si vous avez une colonne "image", vous voudrez peut-être l'inclure dans le formulaire. -->
